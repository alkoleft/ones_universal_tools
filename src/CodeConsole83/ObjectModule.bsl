...
процедура СоздатьКолонкиТЗ(Форма, ИмяТаблицы, Колонки) Экспорт 
	...
	Для Каждого Колонка из Колонки цикл
		...
		Если Элемент = Неопределено Тогда
			...
			Элемент.ОграничениеТипа = Новый ОписаниеТипов(Колонка.ТипЗначения, , "NULL");
			Элемент.ПутьКДанным = СтрШаблон("%1.%2", ИмяТаблицы, Колонка.Имя);
			
		КонецЕсли;
		
		Если Элемент.ОграничениеТипа.СодержитТип(Тип("Булево")) И Элемент.ОграничениеТипа.Типы().Количество() = 1 Тогда 
		...
		Элемент.Заголовок = ?(ЕстьСвойство(Колонка, "Заголовок"), Колонка.Заголовок, Колонка.Имя);
