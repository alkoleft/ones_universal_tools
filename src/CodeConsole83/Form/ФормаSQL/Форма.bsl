...
Функция ВыполнитьЗапросНаСервере(Знач ТекстЗапроса, Знач ПараметрыЗапроса, Знач ПараметрыСоединения, Знач УникальныйИдентификатор)
	
	Результат = ВыполнитьЗапросSQL(ТекстЗапроса, ПараметрыЗапроса, ПараметрыСоединения);
	
	Если НЕ Результат.Выполнено Тогда 
		Возврат Результат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Результат.Данные) Тогда 
		Возврат Результат;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ВыполнитьЗапросSQL(ТекстЗапроса, ПараметрыЗапроса, ПараметрыСоединения)
...
Функция ЗагрузитьТаблицу(RecordSet)
	...
	Для i = 0 По КоличествоКолонок Цикл
		...
		ТипКолонки = ПолучитьТип(RecordSet.Fields(i).Type);
		ИмяКолонки = RecordSet.Fields(i).Name;
		
		Массив = Новый Массив;
		Массив.Добавить(ТипКолонки);
		ТипыКолонок.Добавить(ТипКолонки);
		
		Если НЕ ЗначениеЗаполнено(ИмяКолонки) Тогда 
			ИмяКолонки = "col" + i; 
		КонецЕсли;
		
		РезультатТЗ.Columns.Add(ИмяКолонки, Новый ОписаниеТипов(Массив));
	...
	Пока НЕ (RecordSet.EOF() ИЛИ RecordSet.BOF()) Цикл
		
		Строка = РезультатТЗ.Добавить();
		
		Для i = 0 По КоличествоКолонок Цикл 
			
			Значение = RecordSet.Fields(i).Value;
			
			Если Значение = Null then
				Продолжить;
			ИначеЕсли ТипЗнч(Значение) = ТипCOMМассив Тогда

				Если ТипыКолонок[i] = ТипДвоичныеДанные Тогда
					Значение = ПолучитьДвДанные(Значение);
				Иначе	
					Значение = Значение.Выгрузить();
				КонецЕсли;
