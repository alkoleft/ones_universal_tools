...
Функция ПараметрыПубликации()
	...
	Параметры.Вставить("version", "v1.0.9");
...
Функция Скопировать(Знач Источник, Знач Приемник) Экспорт 
	...
	ИначеЕсли ТипЗ = Тип("ДанныеФормыКоллекция") Тогда
	#КонецЕсли	
...
Процедура ЗагрузитьЗапросДляОтладкиПродолжение(ДанныеЗагрузкиЗапроса, ДопПараметры) Экспорт 
	...
	Результат = ПолучитьДанныеЗапроса(ДанныеЗагрузкиЗапроса.Адрес, ДанныеЗагрузкиЗапроса.НеУдалятьИзХранилища);
...
&НаСервереБезКонтекста
Функция ПолучитьДанныеЗапроса(Знач АдресДанных, Знач НеУдалятьИзХранилища = Ложь)
	...
	Для Каждого Параметр Из ДанныеЗапроса.Параметры Цикл 
		...
		Если НЕ ПараметрыЗапроса.Свойство(Параметр.Ключ) Тогда 
			...
			ПараметрыЗапроса.Вставить(ОписаниеПараметра);
		...
		Если ТипЗнч(Параметр.Значение) = Тип("ТаблицаЗначений") Тогда 
			...
			ОписаниеПараметра.Вставить("АдресДанных", ПоместитьВоВременноеХранилище(Параметр.Значение));
...
Процедура ОтобразитьПараметрыВФорме(ПараметрыЗапроса)
	...
	Для Каждого Элемент Из ПараметрыЗапроса Цикл 
		...
		Значение = ?(Параметр.Свойство("Значение"), Параметр.Значение, Неопределено);
		
		УстановитьЗначениеПараметра(Элемент.Ключ, Значение, Параметр.ТипЗначения, Параметр.Свойство("АдресДанных"));
...
Процедура УстановитьЗначениеПараметра(Имя, Значение, ТипЗначения = Неопределено, ЭтоТаблица = Ложь)
	...
	Если ЭтоТаблица Тогда
		...
		Если НЕ ЭтоАдресВременногоХранилища(Стр.АдресДанных) Тогда 
...
Функция СкопироватьТекущуюНастройку()
	...
	НоваяСтрока = ДобавитьНастройку(ТекущиеДанные.Тип);
	
	ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущиеДанные, , "Ключ");
	
	Если НЕ ЭтоГруппа(ТекущиеДанные.Тип) Тогда 
		ХранилищеНастроек 	= ТаблицаНастроек(ТекущиеДанные.Тип);
		ХранилищеНовой 		= ХранилищеНастроек.НайтиПоИдентификатору(ПолучитьПараметрыНастройки(НоваяСтрока.Ключ).ИдентификаторДанные);
		ХранилищеКопируемой = ХранилищеНастроек.НайтиПоИдентификатору(ПолучитьПараметрыНастройки(ТекущиеДанные.Ключ).ИдентификаторДанные);
