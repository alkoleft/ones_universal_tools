...
&НаСервереБезКонтекста
Функция Параметры_ИзвлечьИзТекста(ТекстЗапроса)
...
Процедура Параметры_ЗаполнитьНаКлиенте()
	...
	Для Каждого Параметр Из Результат.Параметры Цикл 
		...
		УстановитьЗначениеПараметра(Параметр.Имя, , Параметр.ТипЗначения);
...
Процедура ВыбраннаяНастройка_ПараметрыПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные.ИспользоватьСписок Тогда 
		...
		Элементы.ДеревоНастроекПараметрыЗапросаЗначениеПараметра.ОграничениеТипа = Новый ОписаниеТипов("СписокЗначений");
