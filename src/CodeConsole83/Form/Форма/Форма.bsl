...
Функция СоздатьЗначениеНаСервере(Знач Тип, Знач ПараметрыКонструктора)
	...
	Для Инд = 0 По ПараметрыКонструктора.Количество() - 1 Цикл 
		...
		Если ТипЗнч(Знч) = Тип("Структура") И Знч.Свойство("Тип") И Знч.Свойство("Алгоритм") Тогда 
			ПараметрыКонструктора[Инд] = Вычислить(Знч.Алгоритм);
		КонецЕсли;
...
Функция ПараметрыПередНачаломРедактирования()
	...
	ИначеЕсли Поле.Имя = "ВыбраннаяНастройка_ПараметрыЗначение" Тогда 
		...
		ИначеЕсли Описание.ВыбранныйТипПараметра.СодержитТип(Тип("Граница")) Тогда 

			Данные = Новый Массив;
			
			РеквизитыЗначения = ПолучитьРеквизитыЗначения(Значение, "ВидГраницы, Значение");
			
			Реквизит = Новый Структура("Имя, Тип, Обязательное, Значение", "Значение", "Дата", Истина, РеквизитыЗначения.Значение);
			Данные.Добавить(Реквизит);
			
			Реквизит = Новый Структура("Имя, Тип, Обязательное, Значение", "ВидГраницы", "Строка", Ложь, РеквизитыЗначения.ВидГраницы.Алгоритм);
			...
			Реквизит.СписокВыбора.Добавить("ВидГраницы.Включая", "Включая");
			Реквизит.СписокВыбора.Добавить("ВидГраницы.Исключая", "Исключая");
			
			Данные.Добавить(Реквизит);
			
			ПараметрыОткрытия = Новый Структура("ОписаниеДанных, Заголовок", Данные, "Граница");
		...
		ИначеЕсли Описание.ВыбранныйТипПараметра.СодержитТип(Тип("МоментВремени")) Тогда 

			Данные = Новый Массив;
			
			РеквизитыЗначения = ПолучитьРеквизитыЗначения(Значение, "Дата, Ссылка");
			
			Реквизит = Новый Структура("Имя, Тип, Обязательное, Значение", "Дата", "Дата", Истина, РеквизитыЗначения.Дата);
			Данные.Добавить(Реквизит);
			
			Реквизит = Новый Структура("Имя, Тип, Обязательное, Вид, Значение", "Ссылка", ОписаниеТиповДокументыСсылки(), Ложь, "ПолеФлажка", РеквизитыЗначения.Ссылка);
			Данные.Добавить(Реквизит);
			
			ПараметрыОткрытия = Новый Структура("ОписаниеДанных, Заголовок", Данные, "Момент времени");
			ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, Описание);
			ОткрытьФорму("ВнешняяОбработка.CodeConsole83.Форма.ФормаВводаДанных", ПараметрыОткрытия, ВладелецФормы, УникальныйИдентификатор, , , ОписаниеОповещения);
...
Процедура ПослеВводаЗначения(Значение, Описание) Экспорт
	...
	ИначеЕсли Описание.ВыбранныйТипПараметра.СодержитТип(Тип("МоментВремени")) Тогда
		...
		мПараметрыКонструктора.Добавить(Значение.Дата);
		мПараметрыКонструктора.Добавить(Значение.Ссылка);
		Значение = СоздатьЗначениеНаСервере(Тип("МоментВремени"), мПараметрыКонструктора);
		
	ИначеЕсли Описание.ВыбранныйТипПараметра.СодержитТип(Тип("Граница")) Тогда
		
		мПараметрыКонструктора = Новый Массив;
		мПараметрыКонструктора.Добавить(Значение.Значение);
		мПараметрыКонструктора.Добавить(Новый Структура("Тип, Алгоритм", Тип("ВидГраницы"), Значение.ВидГраницы));
		Значение = СоздатьЗначениеНаСервере(Тип("Граница"), мПараметрыКонструктора);
