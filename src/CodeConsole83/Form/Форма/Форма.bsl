...
&НаСервереБезКонтекста
Функция ЗагрузитьЗначение(Знач СостояниеАдрес, Знач Файл, Знач Формат, Знач Тип)
	...
	Возврат ПоместитьВоВременноеХранилище(ПолучитьОбработку(СостояниеАдрес).ЗагрузитьЗначение(Новый ДвоичныеДанные(Файл), Формат, Тип));
...
Процедура Результат_Загрузить(Команда)
	...
	Адрес = ЗагрузитьЗначение(СостояниеАдрес, Файл, ФорматПоИмениФайла(Файл), "ТаблицаЗначений");
