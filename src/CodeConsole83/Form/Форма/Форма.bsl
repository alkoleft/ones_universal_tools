...
Функция ВыполнитьНастройку(Знач ПараметрыНастройки = Неопределено, ВыводитьРезультат = Истина, Текст = Неопределено, пПараметрыВыполненеия = Неопределено) Экспорт
	...
	Иначе
		...
		ВыполнитьНаСервере = ТипФункции = "Запрос" ИЛИ НЕ ТекущиеДанные.НаКлиенте;
...
&НаКлиенте
Функция ВыполнитьРекурсивноНастройки(Знач СтрНастройка)
...
Функция СохранитьНастройкиНаСервере(ИмяФайла)
	...
	//Для Каждого Стр Из Настройки_Дерево Цикл 
	//	
	//КонецЦикла;
	//
...
Функция ДанныеНастройки(ПараметрыНастройки) Экспорт 
	...
	Возврат Настройки_Детали.НайтиПоИдентификатору(ПараметрыНастройки.ИдентификаторДанные);
...
Функция ВыполнитьПервоначальнуюНастройкуФормы(Знач ИнформацияОПлагинах)
	...
	Для Каждого Вариант Из ВариантыСкриптов Цикл
		...
		ПараметрыТиповНастройки = Новый Структура("Имя, Выполнение, КлиентСервер, Редактор, Иконка");
...
Процедура Настройки_Выполнить(Команда)
	...
	ВыполнитьРекурсивноНастройки(Элементы.Настройки_Дерево.ТекущиеДанные);
