...
Функция ПодготовитьТаблицуКВыводу(Таблица) 
	...
	Для Каждого Колонка Из Таблица.Колонки Цикл 
		...
		ОписаниеКолонок.Добавить(Новый Структура("Имя, ТипЗначения, Заголовок", Колонка.Имя, ТипКолонки, Колонка.Заголовок));
...
Функция МожноВыполнить(ПараметрыНастройки = Неопределено)
	...
	Если ПараметрыНастройки = Неопределено ИЛИ (ПараметрыНастройки.Тип <> 0 И ПараметрыНастройки.Тип <> 1) Тогда 
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполнитьНастройку(Знач ПараметрыНастройки = Неопределено, ВыводитьРезультат = Истина, Текст = Неопределено, пПараметрыВыполненеия = Неопределено) Экспорт
	...
	Если ВыполнитьНаСервере Тогда 
		...
		Если РезультатВыполнения.Выполнено И РезультатВыполнения.Свойство("ЕстьДанные") Тогда 
			...
			Если ВыводитьРезультат Тогда
				...
				ВывестиРезультат_Таблица(РезультатВыполнения.АдресДанных);
	...
	Иначе
		...
		Если РезультатВыполнения.Свойство("Данные") И РезультатВыполнения.Данные <> Неопределено Тогда 
			...
			ВывестиРезультат_Таблица(РезультатВыполнения.Данные);
...
Процедура ИнициализироватьКлиент()
	...
	ИнформацияОПлагинах = Новый Массив;
	...
	ПлагинSQL.Команды.Добавить(Новый Структура("Имя, ИмяКоманды, Действие", "УстановитьСоединение", "УстановитьСоединение", "ВыполнитьКоманду"));
	ИнформацияОПлагинах.Добавить(ПлагинSQL);
	
	ПараметрыТиповНастроек = ВыполнитьПервоначальнуюНастройкуФормы(ИнформацияОПлагинах);
