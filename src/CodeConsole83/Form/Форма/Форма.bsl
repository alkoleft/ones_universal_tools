...
&НаКлиенте
Перем БазовыеОперации;
&НаКлиенте
Перем ПараметрыТиповНастроек;
&НаКлиенте
...
Функция МожноВыполнить(Знач ПараметрыНастройки = Неопределено)
	...
	Плагин = ПараметрыТиповНастроек[ПараметрыНастройки.Тип];
...
Функция ВыполнитьНастройку(Знач ПараметрыНастройки = Неопределено, ВыводитьРезультат = Истина, Текст = Неопределено, пПараметрыВыполненеия = Неопределено) Экспорт
	...
	Если ПараметрыТиповНастроек[ПараметрыНастройки.Тип].Плагин <> Неопределено Тогда 
		
		Плагин = Плагин(ПараметрыТиповНастроек[ПараметрыНастройки.Тип].Плагин);
...
Процедура ИнициализироватьКлиент()
	...
	ИнформацияОПлагинах = Новый Структура;
	
	КлиентскиеПлагины= Новый Массив;
	ПлагинSQL = Новый Структура;

	ПлагинSQL.Вставить("Имя", "SQL");
	ПлагинSQL.Вставить("Подключение", "ФормаSQL");
	ПлагинSQL.Вставить("КодТипа", 3);
	ПлагинSQL.Вставить("Выполнение", Истина);
	ПлагинSQL.Вставить("КлиентСервер", Истина);
	ПлагинSQL.Вставить("Команды", Новый Массив);
	ПлагинSQL.Команды.Добавить(Новый Структура("Имя, ИмяКоманды, Действие", "УстановитьСоединение", "УстановитьСоединение", "ВыполнитьКомандуПлагина"));
	
	КлиентскиеПлагины.Добавить(ПлагинSQL);
	
	КомандыПлагинов = Новый Структура;
	Для Каждого Плагин Из КлиентскиеПлагины Цикл 
		
		Для Каждого Команда Из Плагин.Команды Цикл 
			
			КомандыПлагинов.Вставить(Команда.Имя, Плагин);
			
		КонецЦикла;
		
	КонецЦикла;
	
	ИнформацияОПлагинах.Вставить("КлиентскиеПлагины", КлиентскиеПлагины);
	ИнформацияОПлагинах.Вставить("КомандыПлагинов", КомандыПлагинов);
	
	#КонецОбласти // Плагины
	
	ПараметрыТиповНастроек = ВыполнитьПервоначальнуюНастройкуФормы(ИнформацияОПлагинах.КлиентскиеПлагины);
...
Функция ВыполнитьПервоначальнуюНастройкуФормы(Знач ИнформацияОПлагинах)
	...
	ПараметрыТиповНастроек = Новый Соответствие;	
	
	Для Каждого Вариант Из ВариантыСкриптов Цикл
		
		ПараметрыТиповНастройки = Новый Структура("Имя, Выполнение, КлиентСервер, Редактор");
		...
		ПараметрыТиповНастроек.Вставить(Вариант.КодТипа, ПараметрыТиповНастройки);
	...
	Для Каждого Вариант Из ВариантыСкриптов Цикл
		...
		Если Вариант.КлиентСервер = Истина Тогда 
			ОписаниеЭлемента = Новый Структура("Имя, Тип", "ВыполнятьНаКлиенте", Тип("ПолеФормы"));
			ОписаниеЭлемента.Вставить("ПутьКДанным", "ВыполнятьНаКлиенте");
		...
		ПараметрыТиповНастроек[Вариант.КодТипа].Вставить("ИмяСтраницыКоманд", СтраницаВарианта.Имя);
	КонецЦикла;
	...
	Для Каждого Вариант Из ВариантыСкриптов Цикл
		...
		ПараметрыТиповНастроек[Вариант.КодТипа].Вставить("КомандаДобавить", "Добавить_" + Вариант.Имя);
	...
	Возврат ПараметрыТиповНастроек;
...
Функция Плагин(Знач ОписаниеПлагина)
	...
	Если ТипЗнч(ОписаниеПлагина) = Тип("Строка") Тогда 
		...
		Для Каждого Плагин Из ИнформацияОПлагинах.КлиентскиеПлагины Цикл 
			Если Плагин.Имя = ОписаниеПлагина Тогда 
				ОписаниеПлагина = Плагин;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Процедура Настройки_ДобавитьНастройку(Команда)
	...
	Для Каждого Параметр Из ПараметрыТиповНастроек Цикл 
...
Процедура УстановитьНастройкиИнфтерфейса(ПараметрыНастройки)
	...
	Если ПараметрыНастройки <> Неопределено Тогда 
		ПараметрыИнтерфейса = ПараметрыТиповНастроек[ПараметрыНастройки.Тип];
	КонецЕсли;
