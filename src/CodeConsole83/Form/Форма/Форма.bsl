...
Процедура ИнициализироватьКлиент()
	...
	ПараметрыТиповНастроек = ВыполнитьПервоначальнуюНастройкуФормы();
...
Функция ВыполнитьПервоначальнуюНастройкуФормы()
	...
	Для Каждого Вариант Из ВариантыСкриптов Цикл
		...
		Иначе
			...
			Обработка.СоздатьКнопки(ЭтаФорма, КоманднаяПанельВарианта, Вариант.Команды);
		КонецЕсли;
		
		ПараметрыТиповНастройки = Новый Структура("Имя, Выполнение, КлиентСервер, Редактор");
		ПараметрыТиповНастройки.Вставить("ИмяСтраницыКоманд", СтраницаВарианта.Имя);
