...
Функция ПараметрыПубликации()
	...
	Параметры.Вставить("version", "v1.0.6");
...
&НаКлиенте
Функция ВывестиВТаблицу(Таблица, Коллекция, ИскатьПо = Неопределено)
	
	Таблица = ВыбраннаяНастройка_Параметры;
	
	Для Каждого Элемент Из Коллекция Цикл
		
		Если ИскатьПо <> Неопределено Тогда 
			
			Строки = Таблица.НайтиСтроки(Новый Структура(ИскатьПо, Коллекция[ИскатьПо]));
