...
Функция МожноВыполнить(Знач ПараметрыНастройки = Неопределено)
	...
	//Если ПараметрыНастройки.Тип = 0 ИЛИ ПараметрыНастройки.Тип = 1 Тогда 
	//	Возврат Истина;
	//КонецЕсли;
	
	Плагин = ОписаниеТиповНастроек[ПараметрыНастройки.Тип];
	
	Возврат Плагин <> Неопределено И Плагин.Выполнение;
...
&НаКлиенте
Функция ВыполнитьНастройку(Знач ПараметрыНастройки = Неопределено, ВыводитьРезультат = Истина, Текст = Неопределено, пПараметрыВыполненеия = Неопределено) Экспорт
	...
	ВыполняемыйТекст = ?(НЕ ЗначениеЗаполнено(Текст), ТекущиеДанные.Текст, Текст); 
	
	ОписаниеТипаНастройки = ОписаниеТипаНастройки(ПараметрыНастройки.Тип);
	
	#Область Подготовка
			
	ВыполнитьНаСервере = ОписаниеТипаНастройки.КлиентСервер <> Истина ИЛИ НЕ ТекущиеДанные.ДополнительныеПараметры.НаКлиенте;
	...
	Если ОписаниеТипаНастройки.Плагин <> Неопределено Тогда 
		...
		Плагин = Плагин(ОписаниеТиповНастроек[ПараметрыНастройки.Тип].Плагин);
...
Функция ВыполнитьПервоначальнуюНастройкуФормы(Знач ИнформацияОПлагинах)
	...
	Для Каждого Вариант Из ВариантыСкриптов Цикл
		...
		Если Вариант.КлиентСервер = Истина Тогда 
...
Процедура ВыполнитьЗапросСкрипт(Команда)
	...
	ВыполнитьНастройку(ПараметрыТекущейНастройки, , ПолучитьТекст(Истина));
...
Процедура УстановитьНастройкиИнфтерфейса(ПараметрыНастройки)
	...
	Если ПараметрыНастройки <> Неопределено Тогда 
		ПараметрыИнтерфейса = ОписаниеТиповНастроек[ПараметрыНастройки.Тип];
	КонецЕсли;
	
	Если ПараметрыИнтерфейса = Неопределено Тогда 
		ПараметрыИнтерфейса = Новый Структура("Редактор, Выполнение, ИмяСтраницыКоманд", Ложь, Ложь, "Ошибка");
	Иначе
		Элементы.ГруппаКоманды.ТекущаяСтраница = Элементы.Найти(ПараметрыИнтерфейса.ИмяСтраницыКоманд); // Надо созать вкладку для невалидной настройки		
	КонецЕсли;
	
	Элементы.ВыбраннаяНастройка_Текст.Доступность = ПараметрыИнтерфейса.Редактор <> Ложь;
	Элементы.Выполнить.Доступность = ПараметрыИнтерфейса.Выполнение;
