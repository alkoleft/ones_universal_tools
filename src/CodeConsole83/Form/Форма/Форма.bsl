...
&НаКлиентеНаСервереБезКонтекста
Функция ОписаниеДопустимыеТипы()
	
	Ссылки = ОписаниеТиповВсеСсылки();
	Примитивы = Новый ОписаниеТипов("УникальныйИдентификатор, ВидДвиженияНакопления, Булево, ВидДвиженияБухгалтерии, ВидСчета, Строка, Дата, Null, Число, Граница, ТаблицаЗначений");
...
&НаКлиенте
Функция ВыполнитьНастройку(Знач ПараметрыНастройки, ВыводитьРезультат = Истина, Текст = Неопределено, пПараметрыВыполненеия = Неопределено) Экспорт
	...
	Если РезультатВыполнения.Выполнено Тогда 
		...
		Если ВыводитьРезультат И ЗначениеЗаполнено(РезультатДанные) Тогда
...
Процедура УстановитьОписаниеПараметра(Описание)
	...
	Если НЕ Описание.ЭтоСписок И НЕ Описание.ЭтоТаблица Тогда 
...
Функция ПараметрыПередНачаломРедактирования()
	...
	Если Поле.Имя = "ВыбраннаяНастройка_ПараметрыТипПараметра" Тогда
		...
		Если СписокТипов.Количество() = 0 Тогда 
			СписокТипов.ЗагрузитьЗначения(ДопустимыеТипы.Типы());
		КонецЕсли;
