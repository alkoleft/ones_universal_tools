...
Процедура ЗаполнитьПредставлениеТипаДанных()
	...
	Для Каждого Стр Из ВыбраннаяНастройка_Параметры Цикл 
		...
		ТипЗначения = ТипЗнч(Стр.ЗначениеПараметра);
		Стр.ПредставлениеТипаДанных = ?(ЭтоАдресВременногоХранилища(Стр.АдресДанных), "Таблица значений", ТипЗнч(Стр.ЗначениеПараметра));
		Стр.ИспользоватьСписок = ТипЗначения = Тип("СписокЗначений");
		
		Если НЕ ЗначениеЗаполнено(Стр.ВыбранныйТипПараметра) Тогда
			мТипы = Новый Массив;
			Если Стр.ЗначениеПараметра <> Неопределено И ТипЗначения = Тип("СписокЗначений") Тогда 
				Стр.ВыбранныйТипПараметра = Стр.ЗначениеПараметра.ТипЗначения;
			ИначеЕсли Стр.ЗначениеПараметра <> Неопределено Тогда 
				мТипы.Добавить(ТипЗначения);				
				Стр.ВыбранныйТипПараметра = Новый ОписаниеТипов(мТипы);
			ИначеЕсли Стр.ТипПараметра.Типы().Количество() Тогда 
				мТипы.Добавить(Стр.ТипПараметра.Типы()[0]);
				Стр.ВыбранныйТипПараметра = Новый ОписаниеТипов(мТипы);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПараметрыУстановитьНастройкуРедактирования()
	...
	ЭлементЗначение = Элементы.ВыбраннаяНастройка_ПараметрыЗначение;
	...
	Иначе
		...
		ЭлементЗначение.ОграничениеТипа = ?(ЗначениеЗаполнено(Описание.ВыбранныйТипПараметра), Описание.ВыбранныйТипПараметра, ДопустимыеТипы);
		Описание.Данные.ЗначениеПараметра = ЭлементЗначение.ОграничениеТипа.ПривестиЗначение(Описание.ЗначениеПараметра);
...
Процедура ПараметрыПереключитьРежимСписок()
	...
	ЭтоСписок = ОписаниеПараметра.ЭтоСписок;
	ЭтоСписок = НЕ ЭтоСписок;
	
	Если ЭтоСписок Тогда 
		
		Значение = Новый СписокЗначений;
		Значение.ТипЗначения = ОписаниеПараметра.ВыбранныйТипПараметра;
		
		Если ЗначениеЗаполнено(ОписаниеПараметра.ЗначениеПараметра) Тогда 
			Значение.Добавить(ОписаниеПараметра.ЗначениеПараметра);
		КонецЕсли;
		
	Иначе
		
		Значение = ОписаниеПараметра.ВыбранныйТипПараметра.ПривестиЗначение(?(ОписаниеПараметра.ЗначениеПараметра.Количество(), ОписаниеПараметра.ЗначениеПараметра[0].Значение, Неопределено));
		
	КонецЕсли;
	
	ОписаниеПараметра.Данные.ЗначениеПараметра = Значение;
