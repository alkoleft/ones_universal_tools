...
Функция СохранитьЗначение(Знач СостояниеАдрес, Знач Значение, Знач Формат)
	...
	Возврат ПолучитьОбработку(СостояниеАдрес).СохранитьЗначение(Значение, "1");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СостояниеАдрес = РеквизитФормыВЗначение("Объект").Инициализировать(УникальныйИдентификатор);
...
Функция СтрокаФорматДляДиалога(Форматы)
	...
	Для Каждого формат из Форматы Цикл 
		...
		мФорматы.Добавить(СтрШаблон("%1|%2", формат.Ключ, формат.Значение));
