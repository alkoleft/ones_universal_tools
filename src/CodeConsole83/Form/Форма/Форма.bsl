...
Функция ВывестиТаблицу(Знач Таблица)
	...
	РеквизитФормыВЗначение("Объект").ОбновитьКолонкиТЗ(ЭтаФорма, "РезультатТаблица", РеквизитФормыВЗначение("РезультатТаблица").Колонки, СтруктураТаблицы);
...
Процедура ЗагрузитьПлагины()
	...
	Если ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь, УникальныйИдентификатор) Тогда 
		...
		СостояниеАдрес = ЗагрузитьПлагиныСервер(ВыгрузитьЗначения(ПомещенныеФайлы, "Хранение"), УникальныйИдентификатор);
...
&НаСервереБезКонтекста
Функция ЗагрузитьПлагиныСервер(МассивАдресовОбработок, УникальныйИдентификатор) 
	
	Возврат ВнешниеОбработки.Создать("CodeConsole83").Инициализировать(МассивАдресовОбработок, УникальныйИдентификатор);
...
&НаСервереБезКонтекста
Функция ПолучитьФорматыСохранения(Знач СостояниеАдрес, Знач Тип)
	...
	Возврат ПолучитьОбработку(СостояниеАдрес).ПолучитьФорматыСохранения(Тип);
...
&НаСервереБезКонтекста
Функция ЗагрузитьЗначение(Знач СостояниеАдрес, Знач АдресДанных, Знач Формат, Знач Тип)
	...
	Возврат ПоместитьВоВременноеХранилище(Обработка.ЗагрузитьЗначение(ПолучитьИзВременногоХранилища(АдресДанных), Формат, Тип));
...
Процедура Результат_Сохранить(Команда)
	...
	Форматы = ПолучитьФорматыСохранения(СостояниеАдрес, "ТаблицаЗначений");
...
Процедура ПослеПередачиФайлаНаСервер(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт 
	...
	Если ДополнительныеПараметры.Операция = "Результат_Загрузить" Тогда 
		...
		Адрес = ЗагрузитьЗначение(СостояниеАдрес, Адрес, ФорматПоИмениФайла(ВыбранноеИмяФайла), "ТаблицаЗначений");

		ЗагрузитьТаблицу(Адрес);
