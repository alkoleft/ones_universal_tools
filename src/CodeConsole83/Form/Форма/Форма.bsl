...
Функция СкопироватьТекущуюНастройку()
	...
	СкопироватьНастройку(ТекущиеДанные, Неопределено);
...
Процедура Настройки_ДеревоПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	//Если ПеретаскиваемаяСтрока.Тип=ЭтоКатегория Тогда
	//	Строка.Строки.Сдвинуть(новСтр,-Строка.Строки.Количество()+1);
	//КонецЕсли;
	//Если ПеретаскиваемаяСтрока.Родитель=Неопределено Тогда
	//	ДеревоОбработок.Строки.Удалить(ПеретаскиваемаяСтрока);
	//Иначе
	//	ПеретаскиваемаяСтрока.Родитель.Строки.Удалить(ПеретаскиваемаяСтрока);
	//КонецЕсли;
