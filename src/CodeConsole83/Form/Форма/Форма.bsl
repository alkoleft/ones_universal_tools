...
Функция Параметры_ИзвлечьИзТекста(Знач ТекстЗапроса)
	...
	Для Каждого Параметр Из Запрос.НайтиПараметры() Цикл 
		...
		Если Параметр.ТипЗначения.СодержитТип(Тип("ТаблицаЗначений")) Тогда 
			...
			мПараметры.Добавить(Новый Структура("Имя, ТипЗначения, АдресДанных", Параметр.Имя, Новый ОписаниеТипов));
...
Процедура Параметры_ЗаполнитьНаКлиенте()
	...
	Для Каждого Параметр Из Результат.Параметры Цикл 
		...
		УстановитьЗначениеПараметра(Параметр.Имя, , Параметр.ТипЗначения, Параметр.Свойство("АдресДанных"));
...
Процедура ДеревоНастроекПараметрыЗапросаЗначениеПараметраНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЭтоАдресВременногоХранилища(Данные.АдресДанных) Тогда 
		...
		СтандартнаяОбработка = Ложь;
		ОткрытьФорму("ВнешняяОбработка.КонсольКода83.Форма.ФормаРедакторТаблицы", Новый Структура("АдресДанныхТЗ", Данные.АдресДанных), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
