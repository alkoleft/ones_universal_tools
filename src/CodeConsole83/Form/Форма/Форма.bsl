...
Функция ВыполнитьНастройку(Знач ПараметрыНастройки = Неопределено, ВыводитьРезультат = Истина, Текст = Неопределено, пПараметрыВыполненеия = Неопределено) Экспорт
	...
	ДополнитьСтруктуру(ПараметрыВыполнения, пПараметрыВыполненеия);
	
	ВыполняемыйТекст = ?(Текст = Неопределено, ТекущиеДанные.Текст, Текст); 
...
Процедура УстановитьДоступностьФункционала(ДоступнаяФункциональность)
	...
	Элементы.Результат_ТаблицаКонтекстноеМенюРезультат_Сохранить.Доступность = ДоступностьЭлементов.Загрузить;
	Элементы.Результат_ТаблицаКонтекстноеМенюРезультат_Загрузить.Доступность = ДоступностьЭлементов.Сохранить;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолучитьОбработку().ОбновитьКолонкиТЗ(ЭтаФорма, "Результат_Таблица", Новый Массив, );
...
&НаКлиенте
Процедура Выполнить_ВыделенныйТекст(Команда)
