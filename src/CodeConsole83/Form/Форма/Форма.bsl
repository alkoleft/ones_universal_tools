...
Функция ПараметрыПубликации()
	...
	Параметры.Вставить("version", "v1.12");
...
&НаКлиентеНаСервереБезКонтекста
Функция ЗначениеВXMLСтроку(Значение, НазначениеТипа = Неопределено, Форма = Неопределено) Экспорт 
	
	_Запись = Новый ЗаписьXML;
	_Запись.УстановитьСтроку();
	СериализаторXDTO.ЗаписатьXML(_Запись, Значение, ?(НазначениеТипа = Неопределено, НазначениеТипаXML.Неявное, НазначениеТипа), ?(Форма = Неопределено, ФормаXML.Элемент, Форма));
...
&НаКлиенте
Функция ЗначениеИзXMLСтроки(Значение) Экспорт 
...
&НаСервереБезКонтекста
Функция СоздатьЗначениеНаСервере(Знач Тип, Знач ПараметрыКонструктора)
	
	Для Инд = 0 По ПараметрыКонструктора.Количество() - 1 Цикл 
		
		Знч = ПараметрыКонструктора[Инд];
		//Если ТипЗнч(Знч) = Тип("Структура") И Знч.Свойство("Тип") И Знч.Свойство("Алгоритм") Тогда 
		Если ТипЗнч(Знч) = Тип("Строка") Тогда 
			ПараметрыКонструктора[Инд] = Вычислить(Знч);
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат Новый(Тип, ПараметрыКонструктора);
	
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьРеквизитыЗначения(Знач Значение, Знач РеквизитыСтрока, Знач КакXML = Ложь)
	
	Реквизиты = Новый Структура(РеквизитыСтрока);
	ЗаполнитьЗначенияСвойств(Реквизиты, Значение);
	
	Если КакXML Тогда 
		Возврат ЗначениеВXMLСтроку(Реквизиты);
	КонецЕсли;
	
	Для Каждого Эл Из Реквизиты Цикл 
		
		Реквизиты.Вставить(Эл.Ключ, ПодменаЗначения(Эл.Значение));
...
&НаСервереБезКонтекста
Функция ПодменаЗначения(Значение)
	
	ТипЗнч = ТипЗнч(Значение);
	
	Если ТипЗнч = Тип("ВидГраницы") Тогда 
		
		//Возврат Новый Структура("Тип, Алгоритм, Представление", ТипЗнч, "ВидГраницы." + Значение, Строка(Значение));
		Возврат "ВидГраницы." + Значение;
...
&НаКлиенте
Функция ЗначениеЧерезXML(Тип, ПараметрыКонструктора) Экспорт 
	...
	Если Тип = Тип("Граница") Тогда 
		...
		СтрокаXML = СтрШаблон(
			...
			|	<kind>%2</kind>
			|</Bound>", СериализаторXDTO.XMLСтрока(ПараметрыКонструктора.Дата), ?(ПараметрыКонструктора.ВидГраницы = ВидГраницы.Включая, 0, 1));
		
	КонецЕсли;
	
	Значение = XMLСтрокаВЗначение(СтрокаXML, Тип);
...
Функция ПодготовитьТаблицуКВыводу(Таблица) 
	...
	ДопустимыеТипы = ОписаниеДопустимыеТипыТаблица();
...
#КонецОбласти //Работа_с_таблицей
...
&НаКлиентеНаСервереБезКонтекста
Функция ОписаниеТиповДокументыСсылки() Экспорт
...
Функция ОписаниеДопустимыеТипы(ТолькоСериализуемыеТипы = Истина)
	...
	Примитивы = Новый ОписаниеТипов(
		"Число, Строка, Дата, Булево, УникальныйИдентификатор, Null, Граница, МоментВремени, ВидДвиженияНакопления, ВидДвиженияБухгалтерии, ВидСчета"
		+ ?(ТолькоСериализуемыеТипы, "", ", ТаблицаЗначений")
	);
	
	Возврат Новый ОписаниеТипов(Примитивы, Ссылки.Типы()); 
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ОписаниеДопустимыеТипыТаблица()
...
Функция ПараметрыПередНачаломРедактирования()
	...
	ИначеЕсли Поле.Имя = "ВыбраннаяНастройка_ПараметрыЗначение" Тогда 
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, Описание);
		
		Если Описание.ЭтоТаблица Тогда 
			
			РедактироватьТаблицу(Описание.АдресДанных, ОписаниеОповещения);
			Возврат Истина;
			
		КонецЕсли;
		
		НередактируемыеТипы = НередактируемыеТипы();
		
		Если НередактируемыеТипы[Описание.ВыбранныйТипПараметра.Типы()[0]] <> Неопределено Тогда 

			ОписаниеРедатора = НередактируемыеТипы[Описание.ВыбранныйТипПараметра.Типы()[0]];
			ОписаниеРеквизитов = ОписаниеРедатора.ОписаниеРеквизитов;
			
			ЗапрашиваемыеДанные = СтрСоединить(ВыгрузитьЗначения(ОписаниеРеквизитов, "Имя"), ",");
			
			РеквизитыЗначения = ПолучитьРеквизитыЗначения(Значение, ЗапрашиваемыеДанные);
			
			Для Каждого ОписаниеРеквизита Из ОписаниеРеквизитов Цикл 
				
				ОписаниеРеквизита.Вставить("Значение", РеквизитыЗначения[ОписаниеРеквизита.Имя]);
				
			КонецЦикла;
			
			ПараметрыОткрытия = Новый Структура("ОписаниеДанных, Заголовок", ОписаниеРеквизитов, "Граница");
		...
		ИначеЕсли Описание.ВыбранныйТипПараметра.СодержитТип(Тип("УникальныйИдентификатор")) Тогда
			
			ПоказатьВводСтроки(Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, Описание), Строка(Значение), "Значение унакального идентификатора", 36);
			Возврат Истина;
...
&НаКлиенте
Процедура РедактироватьТаблицу(АдресВременногоХранилища, ОписаниеОповещения)
	
	ИмяОткрываемойФормы = "ВнешняяОбработка.CodeConsole83.Форма.ФормаРедакторТаблицы";
	
	ПараметрыОткрытия = Новый Структура("АдресДанныхТЗ", АдресВременногоХранилища);
	
	Форма = ОткрытьФорму(ИмяОткрываемойФормы, ПараметрыОткрытия, , , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Форма.Инициализировать(ЭтаФорма);

КонецПроцедуры

&НаКлиенте
Функция НередактируемыеТипы()
	
	Если глНередактируемыеТипы <> Неопределено Тогда 
		Возврат глНередактируемыеТипы;
	КонецЕсли;
	
	глНередактируемыеТипы = Новый Соответствие;
	...
	Реквизит = Новый Структура("Имя, Тип, Обязательное, Значение", "Значение", "Дата", Истина);
	ОписаниеРеквизитов.Добавить(Реквизит);
	
	Реквизит = Новый Структура("Имя, Тип, Обязательное, Значение", "ВидГраницы", "Строка", Ложь);
	Реквизит.Вставить("Вид", "ПолеПереключателя");
	Реквизит.Вставить("ВидПереключателя", "ВидПереключателя.Тумблер");

	Реквизит.Вставить("СписокВыбора", Новый СписокЗначений);
	Для Каждого Знч Из ВидГраницы Цикл 
		Реквизит.СписокВыбора.Добавить("ВидГраницы." + Строка(Знч), Строка(Знч));
	КонецЦикла;
	
	ОписаниеРеквизитов.Добавить(Реквизит);
	
	ОписаниеРедактора = Новый Структура;
	ОписаниеРедактора.Вставить("ОписаниеРеквизитов", ОписаниеРеквизитов);
	глНередактируемыеТипы.Вставить(Тип("Граница"), ОписаниеРедактора);
	
	#КонецОбласти //Граница

	#Область МоментВремени
	
	ОписаниеРеквизитов = Новый Массив;
	
	Реквизит = Новый Структура("Имя, Тип, Обязательное", "Дата", "Дата", Истина);
	ОписаниеРеквизитов.Добавить(Реквизит);
	
	Реквизит = Новый Структура("Имя, Тип, Обязательное, Вид", "Ссылка", ОписаниеТиповДокументыСсылки(), Ложь, "ПолеФлажка");
	ОписаниеРеквизитов.Добавить(Реквизит);
	
	ОписаниеРедактора = Новый Структура;
	ОписаниеРедактора.Вставить("ОписаниеРеквизитов", ОписаниеРеквизитов);
	глНередактируемыеТипы.Вставить(Тип("МоментВремени"), ОписаниеРедактора);
	
	#КонецОбласти //МоментВремени
...
&НаКлиенте
Процедура ПослеВводаЗначения(Значение, Описание) Экспорт
	
	Если Значение = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	Если Описание.ВыбранныйТипПараметра.СодержитТип(Тип("УникальныйИдентификатор")) Тогда
	...
	ИначеЕсли Описание.ВыбранныйТипПараметра.Типы().Количество() Тогда 
		...
		ТипЗначения = Описание.ВыбранныйТипПараметра.Типы()[0];
		Если НередактируемыеТипы[ТипЗначения] <> Неопределено Тогда 
			
			ОписаниеРедатора = НередактируемыеТипы[Описание.ВыбранныйТипПараметра.Типы()[0]];
			ОписаниеРеквизитов = ОписаниеРедатора.ОписаниеРеквизитов;

			мПараметрыКонструктора = Новый Массив;
			Для Каждого ОписаниеРеквизита Из ОписаниеРеквизитов Цикл 
				мПараметрыКонструктора.Добавить(Значение[ОписаниеРеквизита.Имя]);
			КонецЦикла;
			
			Значение = СоздатьЗначениеНаСервере(ТипЗначения, мПараметрыКонструктора);
			
		КонецЕсли;
		
	КонецЕсли;
	
	Описание.ЗначениеПараметра = Значение;
	УстановитьОписаниеПараметра(Описание);
