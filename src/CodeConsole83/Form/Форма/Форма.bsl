...
Процедура ЗаполнитьПредставлениеТипаДанных()
	...
	Для Каждого Стр Из ВыбраннаяНастройка_Параметры Цикл 
		...
		Если Стр.ТипПараметра.Типы().Количество() Тогда 
			мТипы = Новый Массив;
...
&НаКлиенте
Процедура ПараметрыУстановитьНастройкуРедктирования()
	...
	Иначе
		...
		Элементы.ВыбраннаяНастройка_ПараметрыЗначение.ОграничениеТипа = ?(ЗначениеЗаполнено(Элемент.ТекущиеДанные.ВыбранныйТипПараметра), Элемент.ТекущиеДанные.ВыбранныйТипПараметра, ДопустимыеТипы);
...
Функция ПараметрыПередНачаломРедактирования()
	...
	Значение = Данные.ЗначениеПараметра;
	...
	Если Поле.Имя = "ВыбраннаяНастройка_ПараметрыТипПараметра" Тогда
		...
		СписокТипов.ЗагрузитьЗначения(Данные.ТипПараметра.Типы());
		
		Если СписокТипов.Количество() = 0 Тогда 
			СписокТипов.ЗагрузитьЗначения(ДопустимыеТипы.Типы());
		КонецЕсли;
		
		СписокТипов.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ПослеВыбораТипа", ЭтаФорма), "Выберите тип");
	...
	ИначеЕсли Поле.Имя = "ВыбраннаяНастройка_ПараметрыЗначение" Тогда 
		...
		Если ЭтоАдресВременногоХранилища(Данные.АдресДанных) Тогда 
			...
			ПараметрыОткрытия = Новый Структура("АдресДанныхТЗ", Данные.АдресДанных);
...
Процедура ПослеВыбораТипа(Результат, ДополнительныеПараметры) Экспорт 
	...
	ПараметрыУстановитьНастройкуРедктирования();
...
Процедура ПараметрыПереключитьРежимСписок()
	...
	Если ЭтоСписок Тогда 
		...
		Значение.ТипЗначения = Данные.ВыбранныйТипПараметра;
