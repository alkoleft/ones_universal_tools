...
Функция ПараметрыПубликации()
	...
	Параметры.Вставить("version", "v1.0.7");
...
&НаСервереБезКонтекста
Функция СоздатьТаблицуИзКоллекции(Колонки, Коллекция) 
...
&НаСервереБезКонтекста
Функция ПреобразоватьРезультатВТаблицу(Результат)
...
Функция ВыполнитьНастройку(Знач ПараметрыНастройки = Неопределено) Экспорт
	...
	ПараметрыВыполнения = ТекущиеДанные.Параметры;
...
Функция ВыполнитьНаСервере(Знач Текст, Знач ДанныеПараметрыВыполнения, Знач ТипФункции, Знач АдресРезультата)
	...
	Для Каждого Стр из ДанныеПараметрыВыполнения цикл
		...
		Если ЭтоАдресВременногоХранилища(Стр.АдресДанныхТЗ) Тогда 
			
			ПараметрыВыполнения.Вставить(Стр.ИмяПараметра, ПолучитьИзВременногоХранилища(Стр.АдресДанныхТЗ));
...
Функция Параметры_ИзвлечьИзТекста(Знач ТекстЗапроса)
	...
	Для Каждого Параметр Из Запрос.НайтиПараметры() Цикл 
		...
		Если Параметр.ТипЗначения.СодержитТип(Тип("ТаблицаЗначений")) Тогда 
			...
			мПараметры.Добавить(Новый Структура("Имя, ТипЗначения, АдресДанныхТЗ", Параметр.Имя, Новый ОписаниеТипов));
...
Процедура Параметры_ЗаполнитьНаКлиенте()
	...
	Для Каждого Параметр Из Результат.Параметры Цикл 
		...
		УстановитьЗначениеПараметра(Параметр.Имя, , Параметр.ТипЗначения, Параметр.Свойство("АдресДанныхТЗ"));
...
&НаКлиенте
Процедура УстановитьЗначениеПараметра(Имя, Значение, ТипЗначения = Неопределено, ЭтоТаблица = Ложь)
	...
	Если ЭтоТаблица Тогда
		...
		Если НЕ ЭтоАдресВременногоХранилища(Стр.АдресДанныхТЗ) Тогда 
			
			Стр.АдресДанныхТЗ = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
			
		КонецЕсли;
		
	Иначе
		
		Стр.АдресДанныхТЗ = Неопределено;
...
&НаКлиенте
Процедура ХранилищеНастроекИзвлечь(ПараметрыНастройки)
...
Процедура ЗагрузитьНастройкиНаСервере(Адрес)
	...
	Если ТипЗнч(вНастройки) = Тип("Структура") Тогда 
		...
		Если вНастройки.Алгоритмы.Колонки.Найти("ПараметрыЗапроса") <> Неопределено Тогда 
			вНастройки.Алгоритмы.Колонки.ПараметрыЗапроса.Имя = "Параметры";
		КонецЕсли;
			
		Если вНастройки.Запросы.Колонки.Найти("ПараметрыЗапроса") <> Неопределено Тогда 
			вНастройки.Запросы.Колонки.ПараметрыЗапроса.Имя = "Параметры";
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Рандом.СлучайноеЧисло() % 7 = 1 Тогда 
...
Процедура ВыбраннаяНастройка_ПараметрыПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные.ИспользоватьСписок Тогда 
...
Процедура ВыбраннаяНастройка_ПараметрыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ВыбраннаяНастройка_ПараметрыИспользоватьСписок" Тогда 
		...
		Данные.ИспользоватьСписок = НЕ Данные.ИспользоватьСписок;
...
&НаКлиенте
Процедура ДеревоНастроекПараметрыЗапросаЗначениеПараметраНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Данные = Элементы.ВыбраннаяНастройка_Параметры.ТекущиеДанные;
	Если ЭтоАдресВременногоХранилища(Данные.АдресДанныхТЗ) Тогда 
		
		СтандартнаяОбработка = Ложь;
		ОткрытьФорму("ВнешняяОбработка.КонсольКода83.Форма.ФормаРедакторТаблицы", Новый Структура("АдресДанныхТЗ", Данные.АдресДанныхТЗ), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
