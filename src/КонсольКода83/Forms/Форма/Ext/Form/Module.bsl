...
Функция ВыполнитьЗапросСкриптНаСервере(Текст, ПараметрыВыполнения, ЭтоЗапрос, АдресРезультата)
	...
	Попытка
		...
		Если ЭтоЗапрос Тогда 
			...
			Результат = ВыполнитьЗапросСЗамером(Текст, ПараметрыВыполнения);
...
&НаСервереБезКонтекста
Функция ВыполнитьЗапросСЗамером(ТекстЗапроса, ПараметрыЗапроса)
	...
	Для Каждого Подзапрос Из СтруктураЗапроса Цикл 
		...
		Если Подзапрос.Тип = "СозданиеВТ" Тогда
			...
			Количество = Результат.Выгрузить()[0][0];
	...
	Возврат Новый Структура("Выполнено, ОписаниеРезультатов, Данные", Истина, ОписаниеРезультатов, Результат.Выгрузить());
КонецФункции
...
&НаСервереБезКонтекста
функция НовыйRegEx(шаблон="")Экспорт 
	...
	РегулярноеВыражениеX.Pattern = шаблон;
	возврат РегулярноеВыражениеX;
КонецФункции

&НаСервереБезКонтекста
Функция ЗапросПолучитьВременныеТаблицыИзТекстаЗапроса(ТекстДляОбработки) Экспорт
	...
	RegExp.Pattern = "ВЫБРАТЬ([^;]*)ПОМЕСТИТЬ\s+([^;\s]*)[^;]*";
	наборы = RegExp.Execute(ТекстДляОбработки);
	
	Для каждого набор Из наборы Цикл
		СтруктураВТ.Вставить(СокрЛП(набор.SubMatches(1)), набор.Value);
	КонецЦикла;
