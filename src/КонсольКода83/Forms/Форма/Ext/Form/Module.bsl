...
Процедура ВыполнитьЗапросСкрипт(Команда)
	...
	Описание.Вставить("Общее", ФорматВремяВыполнения(РезультатВыполнения.ОбщееВремя));
	Описание.Вставить("Строк", РезультатТаблица.Количество());
	
	
	Служебные_ОписаниеРезультат = Новый ФорматированнаяСтрока(БиблиотекаКартинок.УстановитьВремя, ФорматированноеОписание(Описание));
	
	Если РезультатВыполнения.Свойство("ВремяВыполнения") Тогда 
		Описание.Вставить("Выполнения", ФорматВремяВыполнения(РезультатВыполнения.ВремяВыполнения));
	КонецЕсли;
	Если РезультатВыполнения.Свойство("ВремяВывода") Тогда 
		Описание.Вставить("Вывод", ФорматВремяВыполнения(РезультатВыполнения.ВремяВывода));
	КонецЕсли;
	Элементы.Служебные_ОписаниеРезультат.Подсказка = Новый ФорматированнаяСтрока(БиблиотекаКартинок.УстановитьВремя, ФорматированноеОписание(Описание, Ложь));
...
&НаСервере
Функция ВыполнитьЗапросСкриптНаСервере(Текст, ПараметрыВыполнения, ЭтоЗапрос)
	...
	Попытка
		...
		Иначе
			...
			Контекст.Вставить("Таблица", РезультатТаблица);
	...
	Если Результат.Выполнено И ЗначениеЗаполнено(Результат.Данные) Тогда 
		
		ВремяНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();
		ВывестиТаблицу(Результат.Данные);
		ВремяОкончания = ТекущаяУниверсальнаяДатаВМиллисекундах();
...
Процедура ВывестиОписаниеЗначенияТаблицы()
	...
	Значение = Элемент.ТекущиеДанные[Элемент.ТекущийЭлемент.Заголовок];
	...
	Описание.Вставить("Тип", ПолучитьПолноеИмяТипа(ТипЗнч(Значение)));
	
	Служебные_ОписаниеТекущегоЗначения = Новый ФорматированнаяСтрока(БиблиотекаКартинок.ПоказатьДанные, ФорматированноеОписание(Описание));
