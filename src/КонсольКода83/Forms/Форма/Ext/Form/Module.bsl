...
&НаКлиенте
Перем ПараметрыТекущейНастройки;
...
Процедура Скопировать(Источник, Приемник)
...
Функция мОшибка(Ошибка, Результат = Неопределено)
	...
	Результат.Вставить("Описание", Ошибка);
...
Функция ЗначениеЗапросом(Текст, Параметры)
	...
	Скопировать(Параметры, Запрос.Параметры);
...
Процедура ПриЗакрытииКЗ(Текст, ДополнительныеПараметры) Экспорт 
	...
	ВыбраннаяНастройка_Текст = Текст;
КонецПроцедуры
...
Процедура Параметры_ЗаполнитьНаКлиенте()
	...
	ТекстЗапроса = ВыбраннаяНастройка_Текст;
...
Процедура ЗагрузитьНастройкиНаСервере(Адрес)
	...
	Если ТипЗнч(вНастройки) = Тип("Структура") Тогда 
		...
		СкопироватьСодержимое(Дерево, вНастройки.Дерево);
		ЗначениеВРеквизитФормы(Дерево,"Настройки_Дерево");
		
		Настройки_Алгоритмы.Загрузить(вНастройки.Алгоритмы);
		Настройки_Запросы.Загрузить(вНастройки.Запросы);
...
&НаКлиенте
Функция ДанныеНастройки(ПараметрыНастройки)
...
Процедура ПолучитьТекстДляМодуля(Команда)
	...
	ТекстДок = СформироватьТекстДляМодуля(ВыбраннаяНастройка_Текст);
...
Процедура ОчиститьТекстЗапросаОтМусора()
	...
	ВыбраннаяНастройка_Текст = обПропылесоситьТекстЗапроса(ВыбраннаяНастройка_Текст);
...
Функция ПолучитьСвойстваЗначения(Значение)
	...
	Если Метаданные.Перечисления.Содержит(Мета) Тогда 
		...
		Описание.Добавить(МетаЗначение.ПолноеИмя(), "Полное имя", , БиблиотекаКартинок.Изменить);
...
Процедура Служебные_ОписаниеТекущегоЗначенияНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ХранилищеПараметров.ТекущееЗначениеТаблицы_Описание.Количество() = 2 Тогда 
		Для Каждого Эл Из ПолучитьСвойстваЗначения(ХранилищеПараметров.ТекущееЗначениеТаблицы_Описание[0].Значение) Цикл 
			ЗаполнитьЗначенияСвойств(ХранилищеПараметров.ТекущееЗначениеТаблицы_Описание.Добавить(), Эл);
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьНастройкиИнфтерфейса(ПараметрыНастройки)
	...
	ИспользоватьРедактор = ПараметрыНастройки.Тип = 0 ИЛИ ПараметрыНастройки.Тип = 1;
	...
	Элементы.ТекстЗапросаКонтекстноеМенюОткрытьКонструкторЗапроса.Доступность = ПараметрыНастройки.Тип = 0;
	
КонецПроцедуры

&НаКлиенте
Процедура ВывестиДанныеНастройки(ПараметрыНастройки)
	...
	ВыбраннаяНастройка_Текст = Стр.Текст;
...
&НаКлиенте
Процедура СохранитьДанныеНастройки(ПараметрыНастройки)
	...
	Стр.Текст = ВыбраннаяНастройка_Текст;
...
Процедура Настройки_ДеревоПриАктивизацииСтроки(Элемент)
	...
	ВывестиДанныеНастройки(ПараметрыТекущейНастройки);
КонецПроцедуры
